...
| [document-workflows.md](document-workflows.md) | **Document workflows** | Uploads that trigger processing — one seam inside `repo-put!` covering console, shim and S3; runs as the uploader; extract → fields (schema-validated) → form → translations, every output a repository document with provenance. **The first-user path.** *(Proposal.)* |
