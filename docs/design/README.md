...
| [document-workflows.md](document-workflows.md) | **Document workflows** | Uploads that trigger processing — one seam inside `repo-put!` covering console, shim and S3; runs as the uploader; extract → fields (schema-validated) → form → translations, every output a repository document with provenance. **The first-user path.** *(Decided 12 Sep 2026; ✅ built through step 2 — the tools, the `doc-pipeline` plugin and "Run workflow…" (slice 57). Triggers are next.)* |
